...
public class AuditKafkaConsumer {
    ...
    @KafkaListener(topics = "logistics.system.audit.v1", containerFactory = "kafkaListenerContainerFactory")
    public void consumeBatch(List<SystemAuditEventAvro> avroEvents, Acknowledgment ack) {
        ...
        if (!avroEvents.isEmpty()) {
            List<AuditEvent> domainEvents = avroEvents.stream().map(auditAvroMapper::toDomain)
                .toList();
